...
export const topicXResults: readonly ProofResultData[] = [
  {
    ...
    purpose: "For N≥4, records the complete finite-product data obtained after a radial boundary point new at order N has been shown to be N-critical.",
    manuscriptHtml: completeHtml("karp:thm:compression"),
    vocabulary: [
      { term: "Multiplier μ", definition: <>One of λ and its complex conjugate, according to which orientation satisfies the conclusion of the finite product theorem.</> },
      { term: "Argument ϑ=arg₊(μ)", definition: <>The unique representative in (0,2π), retained as a real angle rather than reduced modulo 2π.</> },
      ...
      { term: "Factor arguments uⱼ", definition: <>The real arguments of μᑫ−βⱼ selected in [A,M). Their location in one interval makes the phase equation and arithmetic mean unambiguous.</> },
    ...
    proofSteps: [
      ...
      { title: "Record every analytic input", explanation: <>The theorem retains both product forms, the integer e, the complementary parameters, the equation eϑ+Σuⱼ=2π(r−dp), and the bounds uⱼ∈[A,M). No realization or nesting theorem enters.</> },
    ],
  ...
  },
  {
    ...
    purpose: "Transfers the selected multiplier back to the original upper-half-plane ray while preserving its modulus and denominator data.",
    ...
    takeaway: <>The formulas transfer every scalar inequality proved for μ to the original λ without changing the unique radius defined by Topic IX’s scalar equation.</>,
    sourceIds: ["standard-farey"],
  ...
  },
  {
    ...
    purpose: "Uses Jensen’s inequality for one strictly convex log-sine function to prove the radial bound ρ≤ρ*, with equality exactly when all βⱼ agree.",
    manuscriptHtml: completeHtml("karp:thm:hetero-sharp"),
    vocabulary: [
      { term: "Argument u(β)", definition: <>The unique argument in (0,π) of μᑫ−β. It belongs to [A,M) and increases strictly with β.</> },
      { term: "Log-sine function F", definition: <><span className="math-inline">F(u)=log sin M−log sin(M−u)</span>. Triangle trigonometry identifies F(u(β)) with <span className="math-inline">log(|μᑫ−β|/(1−β))</span>.</> },
    ...
    intuition: <>The phase equation fixes the average of the uⱼ at A+B. Taking moduli and logarithms of the product equation fixes ΣF(uⱼ). Strict convexity then compares the varying arguments with their common mean.</>,
    proofSteps: [
      { title: "Define one strictly convex function", explanation: <>Set F(u)=log sin M−log sin(M−u). Then F″(u)=csc²(M−u)&gt;0 on [A,M), and triangle trigonometry gives F(u(β))=log(|μᑫ−β|/(1−β)).</> },
      { title: "Compute the mean argument", explanation: <>Substitute e=s−dq into the phase equation eϑ+Σuⱼ=2π(r−dp) and expand d(A+B). Both expressions give <span className="math-inline">Σuⱼ=d(A+B)</span>.</> },
      ...
      { title: "Compare with the unique equality radius", explanation: <>The scalar left side is strictly increasing in ρ and crosses sin(A+B) once at ρ*. Therefore (II.6.7) is equivalent to ρ≤ρ*.</> },
    ],
    takeaway: <>For every list satisfying the finite product equation, the phase equation, and the bounds uⱼ∈[A,M), one has ρ≤ρ*. Equality holds exactly when β₁=···=β<sub>d</sub>.</>,
    sourceIds: ["jensen-1906", "karpelevic-1951", "ito-1997"],
    sourceRelation: <>The log-sine comparison is proved in this manuscript from the finite-product data; Jensen’s inequality is the classical analytic ingredient.</>,
  },
] as const;
