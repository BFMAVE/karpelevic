...
export const topicVIIIResults: readonly ProofResultData[] = [
  ...
  },
  {
    ...
    purpose: "For n≥2, shows that every attainable point brings the entire segment from the origin with it.",
    manuscriptHtml: completeTopicVIIIHtml("karp:cor:radial-filling"),
    vocabulary: [
      { term: "Star-shapedness with respect to the origin", definition: <>For <var>n</var>≥2, if λ∈Θ<sub>n</sub>, then <span className="math-inline">tλ∈Θₙ</span> for every <span className="math-inline">0≤t≤1</span>.</> },
    ...
    proofSteps: [
      { title: "Treat λ=1 directly", explanation: <>When λ=1 and <var>n</var>≥2, the segment [0,1] has two vertices and satisfies <span className="math-inline">t[0,1]⊆[0,1]</span> for every 0≤t≤1. The invariant-polytope criterion therefore gives <span className="math-inline">t∈Θₙ</span>.</> },
      ...
      { title: "A finite nontrivial vertex orbit", explanation: <>In the two-dimensional unit-modulus case, equality <span className="math-inline">λP=P</span> makes multiplication by λ permute the vertices. Since λ≠1, a nonzero vertex has an orbit of length <span className="math-inline">2≤k≤n</span>, so λᵏ=1.</> },
      { title: "Take the convex hull of the root orbit", explanation: <>Because λᵏ=1 and λ≠1, the geometric-series identity gives <span className="math-inline">1+λ+⋯+λᵏ⁻¹=0</span>. Thus <span className="math-inline">Q=conv{"{"}1,λ,…,λᵏ⁻¹{"}"}</span> contains 0, satisfies λQ=Q, and has at most n vertices. Hence <span className="math-inline">(tλ)Q=tQ⊆Q</span>.</> },
      { title: "Handle the one-dimensional case", explanation: <>A unit-modulus image segment has the same length as the original segment, so inclusion is equality. Its supporting real line is invariant, forcing λ=±1. The case λ=1 was handled first; for λ=−1, the segment [−1,1] satisfies <span className="math-inline">(−t)[−1,1]⊆[−1,1]</span>.</> },
    ],
    provenance: "Previously known",
    sourceIds: ["dmitriev-dynkin-1946", "swift-1972"],
...
export const topicXResults: readonly ProofResultData[] = [
  {
    ...
    kind: "Theorem",
    title: "Finite-product consequences for a new radial boundary point",
    purpose: "For N≥4, this result records the product equation, exact phase equation, and argument bounds obtained after a radial boundary point new at order N has been shown to be N-critical.",
    manuscriptHtml: completeHtml("karp:thm:compression"),
    vocabulary: [
      { term: "Choice μ∈{λ,λ̄}", definition: <>The choice supplied by the two orientations of an adapted complex coordinate.</> },
      { term: "Common modulus ρ", definition: <>The number <span className="math-inline">ρ:=|μ|=|λ|</span>.</> },
      { term: "Chosen angle ϑ", definition: <>The unique number in (0,2π) satisfying <span className="math-inline">μ=ρe<sup>iϑ</sup></span>, retained as a real angle rather than reduced modulo 2π.</> },
      ...
      { term: "Arguments uⱼ of the factors μᑫ−βⱼ", definition: <>The unique angles in (0,π) satisfying <span className="math-inline">μᑫ−βⱼ=|μᑫ−βⱼ|e<sup>iuⱼ</sup></span>. Their location in [A,M) makes the phase equation and arithmetic mean unambiguous.</> },
      { term: "Factors with βⱼ=0", definition: <>They complete the finite product algebraically; they are not asserted to represent additional relative-interior contacts.</> },
    ],
    intuition: <>For N≥4, Topics I–VII provide exactly three analytic inputs: a finite product equation, a phase equation for chosen real arguments, and the bounds uⱼ∈[A,M). No uniqueness of an invariant polygon or realizing stochastic matrix is required.</>,
    proofSteps: [
      { title: "Establish N-criticality", explanation: <>Proposition II.4.7 turns a radial boundary point in Θ<sub>N</sub>∖Θ<sub>N−1</sub> into an N-critical elliptic contraction.</> },
      { title: "Apply the Part I theorem", explanation: <>Because N≥4, Part I Theorem 1.4 applies and gives consecutive Farey fractions, complementary parameters, the finite product equation, the phase equation, and the factor-argument bounds.</> },
      { title: "Account for complex conjugation", explanation: <>The two possible orientations correspond to multiplication by λ and by λ̄, so μ is one of these two numbers and |μ|=|λ|.</> },
      ...
      { title: "List the required analytic conclusions", explanation: <>The theorem retains both product forms, the integer e, the complementary parameters, the equation eϑ+Σuⱼ=2π(r−dp), and the bounds uⱼ∈[A,M). No realization or nesting theorem enters.</> },
    ],
    takeaway: <>The geometric part of the proof has supplied the finite product equation, phase equation, and argument bounds used in Theorem II.6.1.</>,
    sourceIds: ["karpelevic-1951", "ito-1997"],
    sourceRelation: <>This is a consequence of the Part I theorem proved in Topic VII and the stochastic-to-planar reduction of Topic VIII. Karpelevič and Ito are cited as classical boundary-arc antecedents.</>,
  },
  {
    id: "topic-x-reflection",
    label: "Lemma II.5.2",
    kind: "Lemma",
    title: "Effect of complex conjugation on the Farey interval",
    purpose: "Transfers the choice μ∈{λ,λ̄} back to the original upper-half-plane ray while preserving its modulus and denominator data.",
    ...
    vocabulary: [
      ...
      { term: "Angle formulas after reflection", definition: <>The identities |qx−(q−p)|=A/(2π) and |sx−(s−r)|=dB/(2π) recover the same positive angles used in Topic IX.</> },
    ],
    intuition: <>Complex conjugation sends y to x=1−y. The corresponding Farey endpoints reverse order, but their denominators and the modulus remain unchanged.</>,
    proofSteps: [
      ...
      { title: "Recover the positive angular data", explanation: <>The identities <span className="math-inline">|qx−(q−p)|=A/(2π)</span> and <span className="math-inline">|sx−(s−r)|=dB/(2π)</span> recover exactly the positive angles A and B used in Topic IX.</> },
      { title: "Conjugate the equal-parameter equation", explanation: <>The real parameters and integer exponents are unchanged, so conjugation returns the Ito equation for the original interval.</> },
    ],
    takeaway: <>Every inequality expressed solely in the preserved data <span className="math-inline">q,s,d,e,ρ,A,B</span> transfers from the chosen μ to the original λ. The preserved scalar equation therefore defines the indexed Topic IX radius on the original upper-half-plane interval.</>,
    sourceIds: ["standard-farey"],
    sourceRelation: <>This is a direct calculation using elementary Farey reflection and complex conjugation.</>,
  },
  {
    id: "topic-x-heterogeneous",
    label: "Theorem II.6.1",
    kind: "Theorem",
    title: "Radial inequality under the finite-product and phase hypotheses",
    purpose: "Uses Jensen’s inequality for the strictly convex function F(u)=log(sin M)−log(sin(M−u)) to prove ρ≤ρ*, with equality exactly when all βⱼ agree.",
    manuscriptHtml: completeHtml("karp:thm:hetero-sharp"),
    vocabulary: [
      { term: "Argument u(β)", definition: <>The unique angle in (0,π) satisfying <span className="math-inline">μᑫ−β=|μᑫ−β|e<sup>iu(β)</sup></span>. It belongs to [A,M) and increases strictly with β.</> },
      { term: "Function F", definition: <><span className="math-inline">F(u)=log(sin M)−log(sin(M−u))</span>. The sine rule identifies F(u(β)) with <span className="math-inline">log(|μᑫ−β|/(1−β))</span>.</> },
      { term: "Topic IX comparison radius ρ*", definition: <>The indexed radius on the original upper-half-plane Farey interval defined casewise in the Scope. It is the unique solution in (0,1) of Topic IX’s scalar equation.</> },
      { term: "Equality in Jensen’s inequality", definition: <>Because F is strictly convex, equality holds exactly when all input arguments uⱼ coincide.</> },
    ],
    intuition: <>The phase equation fixes the average of the uⱼ at A+B. Taking absolute values and logarithms of the product equation fixes ΣF(uⱼ). Strict convexity then compares the varying arguments with their common mean.</>,
    proofSteps: [
      { title: "Derive the factor formula by the sine rule", explanation: <>Set <span className="math-inline">z=μᑫ</span>. In the triangle with vertices β, 1, and z, the angles are <span className="math-inline">u(β), π−M, M−u(β)</span>. Hence <span className="math-inline">|z−β|/sin M=(1−β)/sin(M−u(β))</span>, which gives the formula for F(u(β)); at β=0 it gives <span className="math-inline">ρᑫ=sin M/sin(M−A)</span>.</> },
      { title: "Check strict convexity", explanation: <>For <span className="math-inline">F(u)=log(sin M)−log(sin(M−u))</span>, one has <span className="math-inline">F″(u)=csc²(M−u)&gt;0</span> on [A,M).</> },
      { title: "Compute the mean argument", explanation: <>Substitute e=s−dq into the phase equation <span className="math-inline">eϑ+Σⱼ₌₁ᵈuⱼ=2π(r−dp)</span> and expand d(A+B). Both expressions give <span className="math-inline">Σⱼ₌₁ᵈuⱼ=d(A+B)</span>.</> },
      { title: "Turn the product into a sum", explanation: <>Taking absolute values and logarithms in the assumed product equation, and using αⱼ=1−βⱼ&gt;0, gives <span className="math-inline">Σⱼ₌₁ᵈF(uⱼ)=(dq−s)logρ</span>.</> },
      { title: "Check every sign used later", explanation: <>From A&lt;A+B&lt;M&lt;π, the sines of A, B, A+B, M, M−A, and M−A−B are positive.</> },
      { title: "Apply Jensen’s inequality", explanation: <>Every uⱼ and their mean A+B lie in [A,M), where F is strictly convex. Hence dF(A+B)≤(dq−s)logρ, with equality exactly when all uⱼ coincide.</> },
      { title: "Identify the equality case", explanation: <>The explicit derivative <span className="math-inline">du/dβ=ρᑫ sin A/|μᑫ−β|²&gt;0</span> shows that all uⱼ coincide exactly when all βⱼ coincide.</> },
      { title: "Exponentiate the inequality", explanation: <>Substituting F and exponentiating gives the sine-ratio inequality. The preceding positivity check justifies every division and preserves the inequality direction.</> },
      ...
      { title: "Compare with Topic IX’s unique solution", explanation: <>The scalar left side is strictly increasing in ρ and equals sin(A+B) once, at the indexed Topic IX radius ρ*. Therefore (II.6.7) is equivalent to ρ≤ρ*.</> },
    ],
    takeaway: <>For every list satisfying the finite product equation, the phase equation, and the bounds uⱼ∈[A,M), one has ρ≤ρ*. Equality holds in this radial inequality—equivalently, ρ=ρ*—if and only if β₁=···=β<sub>d</sub>. Topic XI proves that this common-parameter case is attained by a stochastic matrix.</>,
    sourceIds: ["jensen-1906", "karpelevic-1951", "ito-1997"],
    sourceRelation: <>The comparison for F is proved in this manuscript from the finite-product data; Jensen’s inequality is the classical analytic ingredient.</>,
  },
] as const;
...
export const topicXIResults: readonly ProofResultData[] = [
  {
    ...
    label: "Lemma II.7.1",
    kind: "Lemma",
    title: "Directed-cycle expansion of the characteristic polynomial",
    purpose: "Derives exactly how pairwise vertex-disjoint directed cycles contribute signs, edge weights, and powers of t to a characteristic polynomial.",
    ...
    vocabulary: [
      ...
      { term: "Weighted adjacency matrix", definition: <>An edge u→v of weight w is stored in Aᵤᵥ, so row u lists the weights of edges leaving u.</> },
    ...
    proofSteps: [
      ...
      { title: "Account for the remaining vertices", explanation: <>After the selected nontrivial cycles and selected loops have been fixed, every other vertex contributes t, giving the power tᴺ⁻ᴸ.</> },
    ],
    takeaway: <>Once all directed cycles are classified, the characteristic polynomial follows by summing over their pairwise vertex-disjoint collections.</>,
    provenance: "Classical result",
    sourceIds: ["coates-1959", "kirkland-smigoc-2022"],
    sourceRelation: <>This is the Leibniz determinant expansion grouped by pairwise vertex-disjoint directed cycles; compare C. L. Coates (1959) and Kirkland–Šmigoc (2022), Theorem 2.2.</>,
  },
  {
    ...
    vocabulary: [
      { term: "Block path", definition: <>A directed path in which every nonterminal vertex has one outgoing edge, of weight one.</> },
      { term: "Within-block return edge", definition: <>The terminal edge of weight β that returns to the first vertex of the same block.</> },
      { term: "Inter-block edge", definition: <>The terminal edge of weight α that enters the next block.</> },
      { term: "Subdivision vertex", definition: <>A new vertex inserted on one inter-block edge to lengthen the cycle through all blocks without adding a choice.</> },
    ],
    intuition: <>At every block terminal, a simple cycle uses either the within-block return edge and closes in that block, or the inter-block edge and continues through all blocks.</>,
    proofSteps: [
      { title: "No inter-block edge means one block cycle", explanation: <>The directed path reaches the terminal, whose only within-block exit is the return edge. The entire q-cycle is forced.</> },
      { title: "The outgoing arcs determine the path", explanation: <>After an inter-block edge enters a block, each intermediate vertex has one outgoing edge, so the path to that block’s terminal is uniquely determined.</> },
      ...
      { title: "All later exits are inter-block", explanation: <>Repeating the same reasoning moves through all d blocks and creates the unique cycle containing every inter-block edge.</> },
    ...
    sourceRelation: <>The lemma is proved directly for the graph specified here. Related reduced-Ito realization graphs appear in Johnson–Paparella (2017) and Kirkland–Šmigoc (2022).</>,
  },
  {
    ...
    purpose: "Constructs a row-stochastic matrix whose characteristic polynomial is exactly the reduced Ito polynomial in the cases s≤dq and s>dq.",
    manuscriptHtml: completeHtml("karp:thm:sparse-realization"),
    vocabulary: [
      { term: "Vertex count N₀", definition: <>The number <span className="math-inline">N₀=max(dq,s)</span> of vertices in the constructed weighted digraph before taking a direct sum with an identity matrix.</> },
    ...
    intuition: <>The within-block cycles contribute the binomial (tᑫ−β)ᵈ, while the cycle through all blocks contributes the remaining αᵈ term. For s&lt;dq its entry vertices shorten that cycle, for s=dq no adjustment is needed, and for s&gt;dq one inter-block edge is subdivided.</>,
    proofSteps: [
      { title: "Build d length-q blocks", explanation: <>Internal edges have weight one. Each terminal has a within-block return edge of weight β and an inter-block edge of weight α, so its outgoing total is α+β=1.</> },
      ...
      { title: "Case s≤dq: choose the entry vertices", explanation: <>Farey adjacency implies s&gt;(d−1)q. Set ℓ₁=s−(d−1)q and the remaining ℓⱼ=q; their sum is s. If s=dq, then every ℓⱼ=q and no route is shortened.</> },
      { title: "Compute the first characteristic polynomial", explanation: <>Subsets of the local cycles contribute (tᑫ−β)ᵈ. The length-s cycle through all blocks contributes −αᵈtᵈᑫ⁻ˢ, yielding equation (II.7.3).</> },
      { title: "Case s>dq: subdivide one inter-block edge", explanation: <>Insert K=s−dq subdivision vertices. A collection of within-block cycles leaves these vertices outside its cycles and therefore contributes the additional factor tᴷ; the cycle through all blocks uses them and has length s.</> },
      { title: "Compute the second characteristic polynomial", explanation: <>The two contributions are tˢ⁻ᵈᑫ(tᑫ−β)ᵈ and −αᵈ, giving equation (II.7.4).</> },
      { title: "Verify stochasticity and the endpoint parameters", explanation: <>All weights are nonnegative and every outgoing total is one. Prove the characteristic-polynomial identity first for 0&lt;α&lt;1; because the matrix entries and characteristic polynomial depend polynomially on α, the identity extends to α=0 and α=1.</> },
      { title: "Take a direct sum to reach order n", explanation: <>If N₀&lt;n, replace the matrix by <span className="math-inline">A⊕I<sub>n−N₀</sub></span>. This preserves every root of the reduced Ito polynomial; the cancellation leading from the uncancelled Ito equation concerns only the excluded root t=0.</> },
    ],
    takeaway: <>For every adjacent pair in Fₙ⁺ and every α∈[0,1], the associated reduced Ito polynomial is the characteristic polynomial of an explicit sparse row-stochastic matrix of order at most <var>n</var>.</>,
    provenance: "Previously known",
  ...
  },
  {
    ...
    kind: "Corollary",
    title: "Stochastic realization of the compact candidate curve",
    purpose: <>Proves <span className="math-inline">Γ<sub>f,g</sub><sup>(n)</sup>⊆Θₙ</span>, including the Farey endpoints and the exceptional order-three real segment.</>,
    manuscriptHtml: completeHtml("karp:cor:attainment"),
    intuition: <>For an interior argument, Topic IX supplies complementary coefficients and a nonzero root of the Ito polynomial. Theorem II.7.3 realizes that root. Its endpoint parameters realize the adjacent roots of unity, and the same order-three family covers the exceptional real segment.</>,
    proofSteps: [
      { title: "Realize every interior point", explanation: <>For x∈(f,g), equation (II.2.9) supplies α,β∈(0,1) with α+β=1, and Proposition II.2.3 proves that γ<sub>f,g</sub><sup>(n)</sup>(x) is a nonzero root of the reduced Ito polynomial. Theorem II.7.3 realizes it in Θₙ.</> },
      { title: "Include the ordinary endpoints", explanation: <>At α=0 and α=1, the endpoint forms of Theorem II.7.3 realize the two endpoint roots of unity identified in Proposition II.2.4.</> },
      { title: "Cover the exceptional real segment", explanation: <>For n=3 and [f,g]=[1/3,1/2], write λ∈[−1,−1/2] and choose α=−λ(λ+1)∈[0,1/4]. Then t(t²−β)−α=(t−1)(t²+t+α), so λ is a root and Theorem II.7.3 realizes it in Θ₃.</> },
      { title: "Use the definition of the compact curve", explanation: <>Definition II.2.5 says that these interior points, endpoint points, and—only in the exceptional order-three case—the real segment are exactly Γ<sub>f,g</sub><sup>(n)</sup>.</> },
    ],
    takeaway: <>Every point of Γ<sub>f,g</sub><sup>(n)</sup> belongs to Θₙ. Its identification with the topological boundary is proved later using the radial upper bound and the remaining topological argument.</>,
    provenance: "Previously known",
    sourceIds: ["ito-1997", "johnson-paparella-2017", "kirkland-smigoc-2022"],
    sourceRelation: <>Karpelevič proved the original eigenvalue-region theorem, and Ito gave a polynomial formulation. Johnson–Paparella later constructed parametrized stochastic matrices realizing the arcs, and Kirkland–Šmigoc further analyzed such realizations. This corollary follows from the construction just proved.</>,
  },
  {
    id: "topic-xi-equal-profile",
    label: "Corollary II.6.2",
    kind: "Corollary",
    title: "Equal finite-product parameters at a radial maximum",
    purpose: "Closes the equality argument deferred from Topic X by comparing the radial maximum with the independently realized point from Topic IX.",
    manuscriptHtml: completeHtml("karp:cor:equal-profile"),
    vocabulary: [
      { term: "Equality in the radial bound", definition: <>The upper comparison gives ρ≤ρ*, while realization and radial maximality give ρ*≤ρ. Hence ρ=ρ*, and the equality condition in Jensen’s inequality applies.</> },
      { term: "Equal parameters", definition: <>All parameters in the finite product agree: β₁=⋯=β<sub>d</sub> and therefore α₁=⋯=α<sub>d</sub>.</> },
    ],
    intuition: <>Topic X gives ρ≤ρ*. Corollary II.7.4 with n=N gives ρ*e<sup>iθ</sup>∈Θ<sub>N</sub>, hence ρ*≤R<sub>N</sub>(θ)=ρ. Equality follows, and the equality condition in Jensen’s inequality forces all finite-product parameters to agree.</>,
    proofSteps: [
      ...
      { title: "First inequality", explanation: <>Strict increase of the scalar left side turns Theorem II.6.1 into <span className="math-inline">ρ≤ρ*</span>, where ρ* is Topic IX’s unique solution of the scalar equation.</> },
      { title: "Independent reverse inequality", explanation: <>Apply Corollary II.7.4 with n=N to obtain ρ*e<sup>iθ</sup>∈Θ<sub>N</sub>. Since ρ=R<sub>N</sub>(θ), the definition of the radial maximum gives <span className="math-inline">ρ*≤ρ</span>.</> },
      { title: "Apply the equality condition in Jensen’s inequality", explanation: <>Thus ρ=ρ*, so equality holds in Theorem II.6.1 and β₁=···=β<sub>d</sub>. Because αⱼ=1−βⱼ, also α₁=···=α<sub>d</sub>.</> },
    ...
    takeaway: <>For N≥4, every non-inherited radial maximum is exactly the point determined by the Ito equation in Topic IX, with β₁=···=β<sub>d</sub> and α₁=···=α<sub>d</sub>. Orders at most three are handled independently in Topic XIII.</>,
    sourceIds: ["karpelevic-1951", "ito-1997"],
    sourceRelation: <>This conclusion is proved in the manuscript; no earlier statement-level source is cited here.</>,
  },
] as const;
