...
const commonImports = [
  {
    ...
    explanation: (
      <>
        defines consecutive Farey endpoints, the denominator labels <i>q</i>, <i>s</i>, the factor count <i>D</i><sub>n</sub>(<i>q</i>)=⌊<i>n/q</i>⌋, the angles <i>A</i>, <i>B</i>, and the unique constant-parameter radius whenever the normalized angle lies in an open Farey interval.
      </>
    ),
  },
  {
    ...
    explanation: (
      <>
        proves that parameters 0≤β<sub>j</sub>&lt;1, with
        {" "}α<sub>j</sub>=1−β<sub>j</sub>&gt;0, satisfying the finite-product
        identity, the real-valued argument identity, and the bounds
        {" "}<i>u</i><sub>j</sub>∈[<i>A</i>, Arg(μ<sup><i>q</i></sup>−1)) have
        radius at most the constant-parameter radius, with equality if and
        only if all β<sub>j</sub> agree.
      </>
    ),
  },
] as const;
...
const comparisonResults: readonly ProofResultData[] = [
  {
    ...
    vocabulary: [
      ...
      },
      {
        ...
        definition: <>The convention is det(<i>z,w</i>)=Im(<span aria-label="complex conjugate of z">z̄</span>{" "}<i>w</i>). For endpoint angles <i>A,C</i>&gt;0 with <i>A+C</i>&lt;π, Δ<sub><i>X,Y</i></sub>(1)&lt;0 exactly when the line through <i>X,Y</i> meets the positive real axis beyond 1.</>,
      },
    ],
    intuition: (
      <>
    ...
    proofSteps: [
      ...
      },
      {
        ...
        explanation: <>Write <i>W=γV</i><sub>*</sub> with γ&gt;1 and <i>V</i><sub>*</sub> on the old segment. Collinearity cancels one determinant term, leaving −(γ−1)<i>R</i><sup><i>b</i></sup>sin <i>A</i>&lt;0. The determinant sign criterion therefore puts the new intercept strictly beyond 1.</>,
      },
      {
      ...
      },
      {
        ...
        explanation: <>When <i>b</i>=1, the factor count changes from <i>n</i>−1 to <i>n</i>, so the preceding unchanged-factor-count argument does not apply. The proof instead writes the radial function of the chord from 1 to a root of unity explicitly and differentiates it with respect to the endpoint angle. The derivative is negative, so the smaller new endpoint angle produces the larger radius.</>,
      },
      {
        title: "Reduce d<b by x↦1−x and conjugation",
  ...
  },
  {
    ...
    prelude: (
      <div className="proof-chapter-reading-note" id="topic-xii-exact-topic-x-input">
        <p>
          ...
          applies to parameters 0≤β<sub><i>j</i></sub>&lt;1, with
          {" "}α<sub><i>j</i></sub>=1−β<sub><i>j</i></sub>&gt;0, when
          ...
          and every <i>u</i><sub><i>j</i></sub> lies in [<i>A</i>, Arg(μ<sup><i>q</i></sup>−1)).
    ...
    intuition: <>Keep the old constant-parameter product, extend its parameter tuple by β<sub>M</sub>=0, and reinterpret the same value as an order-<i>n</i> product with one additional factor. Because the parameters (β,…,β,0) are not all equal, the equality case of Theorem II.6.1 does not occur, so its radius comparison is strict.</>,
    ...
    proofSteps: [
      ...
      {
        title: "Append the zero parameter and rewrite the product",
      ...
      },
      {
        ...
        explanation: <>Subtracting <i>qϑ</i> from the old real-argument sum and adding <i>A=qϑ−2πp</i> changes its right-hand side from 2π(<i>r−mp</i>) to 2π(<i>r−Mp</i>). This proves the new equality with no additional multiple of 2π.</>,
      },
      {
        title: "Place every argument in the permitted interval",
        explanation: <>The upper-half-plane branch <i>u</i>(<i>t</i>)=Arg(μ<sup>q</sup>−<i>t</i>) has positive derivative. The old arguments lie strictly between <i>A</i> and Arg(μ<sup><i>q</i></sup>−1), while the new parameter β<sub>M</sub>=0 has argument exactly <i>A</i>. Hence Topic X applies to the full extended tuple.</>,
      },
      {
    ...
    takeaway: <>If only the factor count changes, then <i>K</i><sub>n−1</sub>(θ)&lt;<i>K</i><sub>n</sub>(θ) whenever θ/(2π) lies in the interior of the unchanged Farey interval.</>,
    sourceIds: ["ito-1997"],
...
const assemblyResults: readonly ProofResultData[] = [
  {
    ...
    vocabulary: [
      {
        ...
        definition: <>Because <i>bc−ad=1</i>, the vectors (<i>b,a</i>) and (<i>d,c</i>) form a basis of ℤ². A vector (<i>n,h</i>) whose slope lies strictly between theirs has positive integer coordinates in this basis.</>,
      },
    ],
    intuition: <>A single step from denominator bound <i>n−1</i> to <i>n</i> can add only fractions whose reduced denominator is exactly <i>n</i>. Between consecutive old fractions the unimodular-basis decomposition forces such a fraction, if present, to be their mediant. Everything else is either an endpoint or remains inside an unchanged interval.</>,
    proofSteps: [
      {
      ...
      {
        title: "Place a new fraction between consecutive old fractions",
        explanation: <>For consecutive fractions <i>a/b&lt;c/d</i> of <i>F</i><sub>n−1</sub>, Farey adjacency gives <i>bc−ad=1</i>. The resulting unimodular basis writes the vector (<i>n,h</i>) of an inserted fraction as <i>m(b,a)+ℓ(d,c)</i> with positive integers <i>m,ℓ</i>.</>,
      },
      {
      ...
      {
        title: "Classify the normalized angle",
    ...
    takeaway: <>The two preceding local comparisons cover every change at an interior normalized angle; at Farey fractions the comparison follows from the defining endpoint value 1.</>,
  ...
  },
  {
    ...
    purpose: "Assemble all local comparisons into Kₙ₋₁(θ)≤Kₙ(θ) for 0≤θ≤π, with the corresponding lower-half-plane comparison obtained by reflection.",
    manuscriptHtml: exactResult("karp:thm:candidate-nesting"),
    prelude: (
      <div className="proof-chapter-reading-note proof-chapter-case-table">
        <table>
          <caption>Five cases in passing from <i>F</i><sub>n−1</sub><sup>+</sup> to <i>F</i><sub>n</sub><sup>+</sup></caption>
          <thead>
            <tr>
              <th scope="col">Position of θ/(2π)</th>
              <th scope="col">Change in the Farey data</th>
              <th scope="col">Comparison</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <th scope="row">Inherited Farey fraction</th>
              <td>Endpoint at both orders</td>
              <td>Equality</td>
            </tr>
            <tr>
              <th scope="row">Newly inserted Farey fraction</th>
              <td>An interior value becomes the endpoint value 1</td>
              <td>Strict</td>
            </tr>
            <tr>
              <th scope="row">Interior of an unchanged interval</th>
              <td>The value of <i>D</i><sub>n</sub>(<i>q</i>) is unchanged</td>
              <td>Equality</td>
            </tr>
            <tr>
              <th scope="row">Interior of an unchanged interval</th>
              <td><i>D</i><sub>n</sub>(<i>q</i>) increases by one</td>
              <td>Strict by Lemma II.8.2</td>
            </tr>
            <tr>
              <th scope="row">Interior of a divided interval</th>
              <td>The mediant is inserted</td>
              <td>Strict by Lemma II.8.1</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    vocabulary: [
      {
        ...
        definition: <>When θ/(2π) lies in an open Farey interval, it is Topic IX&apos;s unique radius satisfying the scalar equation. At every Farey endpoint below π, and separately at θ=π, it is defined to be 1.</>,
      },
      {
    ...
    intuition: <>Instead of comparing two complicated curves directly, insert the old radius into the new scalar equation. The exhaustive classification shows that the new residual there is always nonpositive. Because the new residual increases strictly and crosses zero exactly once, its zero must lie at or to the right of the old radius.</>,
    figure: <NestingFigure kind="defect" />,
    proofSteps: [
      {
        title: "Define Fₙ,θ and prove it is strictly increasing",
        explanation: <>For a fixed θ whose normalized value lies in an open Farey interval, define F<sub>n,θ</sub>(<i>t</i>) from the order-<i>n</i> scalar equation. The two sine coefficients and both exponents are positive, so the displayed derivative is positive for every <i>t&gt;0</i>. Topic IX identifies <i>K</i><sub>n</sub>(θ) as its unique zero.</>,
      },
      {
      ...
      },
      {
        ...
        explanation: <>All interior cases give F<sub>n,θ</sub>(ρ<sub>−</sub>)≤0. Strict increase and F<sub>n,θ</sub>(<i>K</i><sub>n</sub>)=0 force ρ<sub>−</sub>≤<i>K</i><sub>n</sub>. For angles interior to both Farey decompositions, the inequality is strict exactly after a mediant insertion or a factor-count increase. Separately, it is strict at every newly inserted Farey fraction.</>,
      },
      {
        title: "Handle Farey endpoint angles",
        explanation: <>At a new endpoint the new value is 1, while the same normalized angle lay in the interior of an order-(<i>n</i>−1) interval and had radius below 1. At an inherited endpoint both values are 1. The separate convention <i>K</i><sub>n</sub>(π)=1 handles the order-three terminal jump when comparing orders three and four.</>,
      },
      {
        title: "Reflect to the lower half-plane",
        explanation: <>Complex conjugation preserves every modulus, so the same comparison holds on the reflected lower-half-plane ray. The function <i>K</i><sub>n</sub> itself remains defined here on 0≤θ≤π.</>,
      },
    ],
    takeaway: <>For 0≤θ≤π, <i>K</i><sub>n−1</sub>(θ)≤<i>K</i><sub>n</sub>(θ); conjugation gives the corresponding lower-half-plane comparison. Topic XIII uses this inequality when a stochastic extremum is inherited from the preceding order.</>,
  ...
  return (
    ...
    >
      <header>
        ...
        <div>
          <p>Topic XII presents manuscript §II.8. These conventions are used throughout the chapter.</p>
        </div>
      </header>
      <div className="proof-chapter-reading-note">
        ...
        <p>
          ...
          <i> F</i><sub>n</sub><sup>+</sup>, then <i>K</i><sub>n</sub>(θ) is Topic IX’s unique radius satisfying the scalar equation.
          ...
          The separate definition at θ=π records the order-three jump: the nonreal arc tends to radius 1/2 as θ↑π, while the endpoint −1 has radius 1.
        </p>
        <p>
          <strong>Logical status.</strong> The notation <i>K</i><sub>n</sub> denotes the candidate selected by the scalar equation.
          Topic XIII, not the notation itself, proves that this value equals the maximum modulus attained by Θ<sub>n</sub> on the same ray.
        </p>
      </div>
    </section>
  );
  ...
  return (
    <>
      ...
      <ProofDependencyContract
        ...
        provedHere={<p>Lemmas II.8.1 and II.8.2 prove the two strict local comparisons. Lemma II.8.3 shows that these and the endpoint or unchanged cases exhaust Farey refinement. Theorem II.8.4 then obtains <i>K</i><sub>n−1</sub>(θ)≤<i>K</i><sub>n</sub>(θ) for 0≤θ≤π.</p>}
      />
